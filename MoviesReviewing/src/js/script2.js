...
async function fetchAndDisplayMovies() {
    try {
        const response = await fetch("../assets/movies2.json");
        if (!response.ok) {
        ...
        // Extract comedyMovies array
        const comedyMovies = data.comedyMovies; 

        // Debugging log to verify data
        console.log("Fetched comedy movies:", comedyMovies);
...
function loadMovies(comedyMovies) {
    const container = document.getElementById("comedy-Movie-list");
    ...
    const moviesToDisplay = comedyMovies.slice(0, 8);
    //const moviesToDisplay = comedyMovies.slice(4, 5); // Only show the 5th movie
...
function searchMovies() {
    ...
    if (foundMovies === 0) {
        document.getElementById('comedy-Movie-list').innerHTML = "<p>No matching movies found.</p>";
    }
}
...
function backToMovies() {
    ...
    document.getElementById('comedy-Movie-list').style.display = 'flex';
}
